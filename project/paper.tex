...
\author{
Vivien Cheng, Harry Voorhis, Brian Donohugh\\
Stanford University\\
...
We estimate gravimetric capacity as $Q = nF / (3.6 \cdot MW)$ with $n = 2$ and $F = 96{,}485$ C/mol. We rank molecules by a weighted score prioritizing potential over capacity with a small penalty for physicochemical outliers (50\% normalized potential, 35\% normalized capacity, 15\% outlier penalty), and identify Pareto-optimal candidates.
...
Figure~\ref{fig:ablations}b shows predicted redox potential against the capacity proxy for all 494 molecules, with Pareto-optimal candidates in red. The Pareto front reveals an explicit tradeoff: very high-potential molecules tend to have higher molecular weight (lower capacity). Top-ranked candidates cluster in the high-potential, moderate-capacity region. This two-objective view is more informative than sorting by potential alone. Note that the screening scores all 494 molecules, including the 395 training molecules, so predictions for training-set molecules are in-sample and should be validated experimentally. Full screening results are available in the code repository.
